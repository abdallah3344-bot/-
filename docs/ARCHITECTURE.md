...
|---|---|---|
| 1 | المعمارية + قاعدة البيانات + المصادقة | ✅ |
...
## 8. قرارات تغيّرت أثناء التنفيذ

التوثيق الصادق يشمل ما اكتُشف بالاختبار لا ما خُطّط له فقط.

### 8.1 قيد الخصم على الفواتير

كان القيد `discount <= subtotal` على جدول `invoices`. فشل دائمًا:
رأس الفاتورة يُدرج قبل بنودها، وفي تلك اللحظة `subtotal = 0`، وقيود
`CHECK` لا تُؤجَّل في PostgreSQL. نُقل ضمان الثبات إلى محفّز إعادة
الاحتساب الذي يقصّ الخصم عند المجموع بعد استقرار البنود، مع تحقّق
في الخادم يُعطي المستخدم رسالة واضحة قبل الحفظ.

### 8.2 موضع تصدير النسخة الاحتياطية

كان التصدير يجري في المتصفح بخمسة وعشرين طلبًا متتاليًا. كان بطيئًا
وينقطع. نُقل إلى مسار على الخادم: طلب واحد من المتصفح، والاستعلامات
قرب قاعدة البيانات. لا يزال يمرّ بعميل المستخدم فلا يتجاوز RLS.

### 8.3 موضع توليد التنبيهات

كان التوليد في صفحة التنبيهات وحدها، فيبقى عدّاد الترويسة قديمًا لأن
التخطيط يُصيَّر قبل الصفحة. نُقل إلى التخطيط العام وكُبح في قاعدة
البيانات إلى مرة كل عشر دقائق لكل مستخدم.

### 8.4 الحد الأدنى لعرض أبناء الشبكات

ظهر تمرير أفقي على الموبايل بعد إدخال بيانات حقيقية: أبناء شبكات CSS
عرضهم الأدنى `auto`، فالأسماء العربية الطويلة توسّع العمود بدل أن
تُقصّ. ضُبط `min-width: 0` على أبناء الشبكات والصناديق المرنة، مع
`overflow-x: clip` على `html` لمنع الشريط الجانبي المنزلق من توسيع
مساحة التمرير في RTL.

### 8.5 التحقق من المدخلات في النماذج

تحقّق المتصفح الأصلي كان يحجب الإرسال برسالة بلغة نظام المستخدم
ويتجاوز رسائلنا العربية. أُضيف `noValidate` لكل النماذج ليكون التحقق
عبر zod وحده برسائل عربية موحّدة.

### 8.6 كشف البريد عند تسجيل الدخول

النسخة الأولى من `resolve_login_email` كانت تُرجع البريد المقابل لاسم
المستخدم لأي زائر، فتُمكّن من تعداد الحسابات. النسخة الحالية لا تُرجع
البريد إلا بعد التحقق من كلمة المرور نفسها.

---

## 9. الربط بلوحة تراخيص المصري جروب

### 9.1 موقع النظام في المنظومة

النظام مسجَّل في لوحة التراخيص كبرنامج مستقل بالمعرّف `law-office`،
مثل بقية برامج المجموعة. اللوحة تُصدر التراخيص وتربط الأجهزة وتوافق
على الطلبات التجريبية؛ والنظام يسأل عن حالته فقط.

بيانات المكتب تبقى في قاعدته المستقلة تمامًا. اللوحة لا ترى أي قضية
ولا موكل ولا مبلغ — تعرف فقط أن نسخة بهذا المعرّف تسأل عن ترخيصها.

### 9.2 أين يُتخذ قرار السماح

القرار يأتي من رد `verify_license` في اللوحة عند كل طلب، ولا يُقرأ من
قاعدة بيانات النظام إطلاقًا. البديل المغري — تخزين «مرخَّص: نعم» في
جدول والاكتفاء بقراءته — يجعل أي مستخدم مسجَّل قادرًا على فتح النظام
بكتابة صف، فرُفض.

`license_state` يحفظ المُدخلات وحدها: مفتاح الترخيص ومعرّف النسخة
ورقم الجوال. سياسة RLS تقصر تعديلها على صاحب `settings.update`، وحقول
`last_state` و`last_verified_at` للعرض والتشخيص فقط.

### 9.3 السلوك عند انقطاع الاتصال

تعذُّر الوصول لخادم التراخيص يُبقي النظام عاملًا مع تنبيه ظاهر، ولا
يعطّله. تعطيل مكتب محاماة عن جلسة أو موعد تقادم بسبب انقطاع شبكة عن
خادم تراخيص ضرر أكبر من أي نفع. أما الرفض الصريح — منتهٍ أو مفتاح غير
صحيح — فيُغلق كل المسارات فورًا.

### 9.4 الاختبار

الاختبارات تعمل على خادم تراخيص محاكٍ محلي، لا على اللوحة الحقيقية:
إنشاء تراخيص وهمية في نظام تجاري حيّ غير مقبول، وربط الاختبارات بخادم
خارجي يجعلها تفشل عند انقطاع الشبكة. المحاكي يردّ بنفس صيغة
`verify_license`، فتُغطّى كل الحالات بما فيها الانقطاع نفسه.

---

## 10. السياق الفلسطيني

النظام موجَّه لمكاتب المحاماة في فلسطين، لا لسوق عربي عام. أثر ذلك
يظهر في البيانات المرجعية لا في بنية النظام:

- **المحافظات** ثابتة في `src/lib/constants/palestine.ts` وتُعرض قائمة
  اختيار. كانت حقلًا حرًّا، فكان كل مستخدم يكتبها بصيغة مختلفة ويُفسد
  التصفية والتقارير.
- **المحاكم وأنواع القضايا** بيانات في القاعدة لا ثوابت في الكود:
  تركيبة المحاكم تتغيّر، والمكتب أدرى بما يخصّه. تُعدَّل من
  «الإعدادات ← جداول المراجع».
- **العملة الافتراضية** الشيكل، ومعها الدينار الأردني والدولار
  لأن الثلاثة متداولة عمليًا في فلسطين.
- **ضريبة القيمة المضافة** 16%.

ما وُطِّن هو القيم الابتدائية فقط. لا شيء في الكود يفترض فلسطين
افتراضًا صلبًا: تبديل قائمة المحافظات وبيانات المحاكم يكفي لتشغيل
النظام في ولاية قضائية أخرى.
