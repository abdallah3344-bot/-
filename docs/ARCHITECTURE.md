...
## 11. أدوات لائحة الدعوى

### 11.1 لماذا لا نولّد «نماذج معتمدة»

لم يُعثر على كتالوج نماذج رسمي منشور لمجلس القضاء الأعلى، والملزِم في
القانون هو *مضمون* اللائحة لا شكلها. توليد لائحة من عندنا وتسميتها
«معتمدة» يعني أن مستندًا قد يُردّ في المحكمة بسبب النظام — وهذا ضرر
على موكّل، لا مجرّد عيب برمجي.

فالنظام لا يؤلّف نصًّا قانونيًا إطلاقًا. يفعل ثلاثة أشياء فقط:

1. **يملأ قالب المكتب** — القالب مستند المحامي، والنظام ينسخ إليه
   بيانات القضية بلا أخطاء نسخ يدوي.
2. **يفحص اكتمال البيانات** التي يوجبها القانون — ويقول «هذا الحقل
   فارغ» لا «هذه اللائحة صحيحة».
3. **يرتّب البيانات** لإعادة إدخالها في ميزان.

### 11.2 محرّك القوالب

استبدال نصّي بحت بلا تنفيذ تعابير ولا منطق داخل القالب. مقصود: القالب
مستند يوقّعه محامٍ، فلا يصحّ أن يُنتج النظام فيه فرعًا أو حسابًا لا
يراه من يوقّع.

الحقل بلا بيانات يظهر `__________` لا اسم الحقل ولا قيمة مخمّنة،
ويُنبَّه عليه قبل الطباعة. ظهور اسم الحقل في مستند مطبوع خطأ مهني،
وتخمين القيمة أسوأ.

قوالب Word تُملأ **في الخادم**: ملفات القوالب في حاوية خاصة، ولا يصحّ
أن تمرّ عبر المتصفح لمجرد تعبئتها.

### 11.3 بيانات المادة 52

أُضيفت كحقول مستقلة (`claim_requests`, `claim_arose_at`,
`property_description`, وحقول الأهلية ومحل العمل للأطراف) لا كنصّ حر،
لأن الفحص الآلي للاكتمال يحتاج حقلًا يُسأل عنه. البنود الشرطية تُعلَّم
«غير منطبق» لا «ناقص»، وإلا صار المدقّق يشكو دائمًا فيُتجاهَل.

### 11.4 ميزان

لا تكامل: النظام مغلق ولا صيغة استيراد منشورة له. ادّعاء التوافق مع
نظام لم نره كذبٌ يكتشفه المستخدم عند أول محاولة، فالمكتوب في الواجهة
والتوثيق أن هذه ورقة ترتيب للنسخ لا أكثر.
